  ...
  let isLoading = false;
	...
	async function handleSignup(event) {
		...
		try {
			...
			// Use credentials provider for email/password signup
      ...
      <ul class="mt-4 ml-4 space-y-12 pt-10 w-[65%] font-roboto">
		...
		<div class="w-full max-w-md space-y-8 rounded-lg bg-[#f8f7f3] p-10 shadow-lg border border-gray-200">
          ...
          <!-- Last Name Group -->
					...
					<div class="space-y-1">
						<p class="text-sm text-gray-900">Email address</p>
						<input
							class="w-full rounded-md border border-gray-300 bg-[#f8f7f3] p-2 placeholder:text-gray-200"
							type="email"
					...
					<div class="relative pt-8">
						<button
							type="submit"
							disabled={isLoading}
							class="focus:shadow-outline w-full rounded-lg bg-[#ff6823] px-4 py-2 font-roboto text-sm text-white hover:bg-[#ff6823] focus:outline-none disabled:opacity-50"
						>
							{#if isLoading}
				...
				<p class="text-center text-[13px] font-normal font-roboto text-gray-500">
					Already have an account? <a href="/login" class="text-[#ff6823] text-xs font-bold hover:underline"
						>Sign in</a
					>
				</p>
			</div>
